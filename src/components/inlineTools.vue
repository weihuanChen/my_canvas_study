...
<template>
  <div>
    <div style="display: inline-block" v-if="areaShow">
      <span class="title">基础元素</span>
...
export default {
        ...
        _vm.graphNumberList.map((item) => {
          if (item.value !== null && newVal === item.value) {
            ...
            if (item.value === 9) {
              ...
              this.areaShow = true;
              this.drawHandler.setPath(false, {
                name: item.label,
                value: item.value,
                text: item.label,
              });
  ...
  computed: {
    ...mapState(useSelectStore, ["selectItem"]),
  },
  created() {
  ...
  mounted() {
    console.log("render");
    this.areaShow = false;
    this.updateTools(this.selectItem);
    // this.drawingLineModeSwitch(false);
    //this.drawingLineModeSwitch(true);
  },

  methods: {
    updateTools(selectType) {
      let _vm = this;
      _vm.graphNumberList.map((item) => {
        if (item.value !== null && selectType === item.value) {
          _vm.graphName = item.label;
          //   console.log(item);
          //电缆隐藏掉图形工具
          if (item.value === 9) {
    ...
    addCircle(option) {
      ...
      const block = new fabric.Group([text, circle], {
        ownerObject: [circle, text],
        text: addText + "组",
        value: _this.selectType,
      });
    ...
    addRect(option) {
      ...
      const block = new fabric.Group([text, rect], {
        ownerObject: [rect, text],
        text: addText + "组",
        value: _this.selectType,
      });
      this.canvas.c.add(block);
    ...
    drawingLineModeSwitch(isArrow) {
      console.log(this.isDrawingLineMode);
