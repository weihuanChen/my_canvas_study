...
function initializeLineDrawing(canvas, defaultPosition) {
  ...
  let isArrow = false;
  let isPath = false; //如果是业务属性
  let pathObject = null;//业务属性
  let lineToDraw;
  let pointer;
  let pointerPoints;
  ...
  canvas.on('mouse:down', (o) => {
    ...
    const { name, value, text } = pathObject;
    if (!isPath) {
      //todo 非路径情况下原本不需要添加业务属性
      //lineToDraw = new NodeHandler(pointerPoints, lineObject);
      lineObject.name = name;
      lineObject.value = value;
