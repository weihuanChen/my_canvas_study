...
class Editor extends EventEmitter {
  ...
  unGroup() {
    ...
    activeObject.getObjects().forEach((item: fabric.Object, index: number) => {
      console.log(activeObject.ownerObject);

      item.set('id', uuid());
      //打入业务属性
      item.set('text', activeObject.ownerObject[index].text)
  ...
  group() {
    ...
    activegroup.clone((newgroup: fabric.Group) => {
      let text = objectsInGroup[0].text;
      if (text.indexOf('组') == -1) {
        text += '组'
      }
      newgroup.set('id', uuid());
      newgroup.set('text', objectsInGroup[0].text)
      newgroup.set('value', objectsInGroup[0].value)
      newgroup.set('name', objectsInGroup[0].name)
      this.canvas.remove(activegroup);
      objectsInGroup.forEach((object) => {
        this.canvas.remove(object);
      });
      //把组合的元素记录下来
      newgroup.ownerObject = objectsInGroup
  ...
  getJson(c:fabric.Canvas) {
    return this.canvas.toJSON(['id', 'gradientAngle', 'selectable', 'hasControls']);
    //return c.toJSON(['id', 'gradientAngle', 'selectable', 'hasControls'])
  }
